...
function colorBG {
    color=`getBGColorByName "$2"`
    echo -en "\033$color";
    echo -n "$1";
    echo -en "\033[0m"
}

function colorFGStream {
    local IFS_BAK=$IFS;
    local IFS=`echo -e "\n\r"`;
    while read msg; do colorFG "$msg" "$1" "$2"; echo ''; done
    local IFS=$IFS_BAK;
}

function colorBGStream {
    local IFS_BAK=$IFS;
    local IFS=`echo -e "\n\r"`;
    while read msg; do colorBG "$msg" "$1"; echo ''; done
    local IFS=$IFS_BAK;
} 

function getFGColorByName {
    if [ -n "$2" ]; then
